-- OICAR Database - Complete Clean Setup Script
-- This script will handle all dependencies properly to avoid column reference errors

USE master;
GO

-- Drop database if it exists and recreate
IF EXISTS (SELECT name FROM sys.databases WHERE name = 'SnjofkaloDB')
BEGIN
    ALTER DATABASE SnjofkaloDB SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE SnjofkaloDB;
END
GO
...
PRINT 'Database SnjofkaloDB created successfully with all tables, indexes, and sample data!'; 