...
-- Comments about encryption implementation:
/*
ENCRYPTION NOTES:
=================
...
2. Field Size Considerations:
   - Base64 encoding increases size by ~33%
   - "ENC:" prefix adds 4 characters
   - AES encryption with padding can add up to 16 bytes
   - Original sizes were multiplied by 3-4x to be safe
   
3. Search Limitations:
   - Cannot use WHERE clauses on encrypted fields directly
   - Indexes on encrypted fields are less useful for searching
   - Must decrypt all records to search (performance impact)
   - Consider implementing searchable encryption for high-volume scenarios
   
4. Performance Impact:
   - Encryption/decryption adds CPU overhead
   - Larger column sizes increase storage and memory usage
   - Consider caching decrypted data for frequently accessed records
   
5. Security Benefits:
   - PII data is encrypted at rest
   - GDPR compliance for data protection
   - Prevents data exposure from database dumps/backups

6. GDPR Anonymization Features:
   - RequestedAnonymization: Boolean flag for user requests
   - AnonymizationRequestDate: Tracks when request was made (30-day GDPR deadline)
   - AnonymizationReason: User's reason for requesting anonymization
   - AnonymizationNotes: Admin notes or additional information
   - Constraint ensures data consistency (flag=true requires date)
   - Indexes optimized for admin queries (filtered indexes for performance)

7. MARKETPLACE FEATURES:
   - User-Generated Content: Users can list their own items for sale
   - Two-Tier Item System: Admin items (auto-approved) vs User items (need approval)
   - Commission Tracking: Platform takes commission on user sales
   - Seller Management: Seller profiles, ratings, and verification system
   - Financial Tracking: Complete audit trail of sales, commissions, and payments
   - Item Approval Workflow: Admin review process for user-listed items
   - Seller Profiles: Business info, ratings, verification status, suspension management
*/