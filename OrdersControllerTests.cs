...
namespace SnjofkaloAPI.Tests
{
    public class OrdersControllerTests
    {
        ...
        [Fact]
        public async Task GetMyOrders_AuthenticatedUser_ReturnsOrders()
        {
            // Arrange
            SetupUserContext(userId: 1);
            var expectedResponse = new ApiResponse<PagedResult<OrderListResponse>>
            {
                Success = true,
                Data = new PagedResult<OrderListResponse>
                {
            ...
            _mockOrderService.Setup(x => x.GetUserOrdersAsync(1, 1, 20))
                .ReturnsAsync(expectedResponse);
        ...
        public async Task GetOrders_AdminUser_ReturnsAllOrders()
        {
            ...
            _mockOrderService.Setup(x => x.GetOrdersAsync(1, 20))
                .ReturnsAsync(expectedResponse);
        ...
        [Fact]
        public async Task GetOrderAnalytics_AdminUser_ReturnsAnalytics()
        {
            // Arrange
            SetupUserContext(userId: 1, isAdmin: true);
            ...
            var expectedResponse = new ApiResponse<object>
            {
                ...
                Data = new
                {
                    ...
                    TotalOrders = 150,
                    TotalRevenue = 45750.50m,
                    AverageOrderValue = 305.00m,
                    TotalItemsSold = 425,
                    UniqueCustomers = 89,
                    MarketplaceBreakdown = new
                    {
                        StoreOrders = new { Count = 95, Revenue = 28500.25m },
                        MarketplaceOrders = new { Count = 55, Revenue = 17250.25m }
                    },
                    StatusBreakdown = new[]
                    {
                        new { Status = "Pending", Count = 12 },
                        new { Status = "Processing", Count = 8 },
                        new { Status = "Shipped", Count = 15 },
                        new { Status = "Delivered", Count = 110 },
                        new { Status = "Cancelled", Count = 5 }
                    }
                }
            };

            _mockOrderService.Setup(x => x.GetOrderAnalyticsAsync(fromDate, toDate))
                .ReturnsAsync(expectedResponse);

            // Act
            var result = await _controller.GetOrderAnalytics(fromDate, toDate);
